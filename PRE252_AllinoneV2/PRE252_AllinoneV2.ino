...
//-------------------------Line Notify--------------------------------------
  ...
  if(digitalRead(23) == 1)
  {
    Lbutton = "ระบบปั้มน้ำรั่ว กรุณาตรวจสอบด่วน !!" ;
  }
  else
  {
    Lbutton = "ระบบปั้มน้ำปิดสนิท ไม่มีการรั่วไหล" ;
  }
...
//--------------------Case แจ้งเตือน ความชื้นต่ำ -------------------
  ...
  if(mosafter <= 50)
  {
      if(n==1)
      {
          ...
          {
            warn = 1 ;
            Line2 = "***** คำเตือน !!!! ความชื้นต่ำ กรุณาเปิดระบบรดน้ำด้วย *****\n***** ถ้าหากรดน้ำแล้ว ขออภัย กรุณารดน้ำอีกครั้ง เพราะความชื้นไม่ถึงเกณฑ์ ***** " ;
            LINE.setToken(LINE_TOKEN);
            LINE.notify(Line1);
            LINE.notify(Line2);
            LINE.setToken(LINE_TOKEN2);
...
BLYNK_WRITE(V7)
{
  ...
  if(pinValue == 1 )
  {
    ...
    Blynk.virtualWrite(V10, pinValue);
    Line3 = " .....เปิดระบบรดน้ำต้นไม้แล้ว ปั้มน้ำกำลังทำงาน กำลังรดน้ำต้นไม้ของท่าน..... " ;
    LINE.setToken(LINE_TOKEN);
    LINE.notify(Line3);
    LINE.setToken(LINE_TOKEN2);
    LINE.notify(Line3);
    ...
    digitalWrite(23,LOW);
    pinValue = 0 ;
    Line3 = " .....ปิดระบบรดน้ำต้นไม้แล้ว ปั้มน้ำปิดการใช้งาน..... " ;
    LINE.setToken(LINE_TOKEN);
    LINE.notify(Line3);
    LINE.setToken(LINE_TOKEN2);
    LINE.notify(Line3);
